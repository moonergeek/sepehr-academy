...
const App = () => {
  ...
  const [placeHolderForMaghalat, setPlaceHolderForMaghalat] = useState(
    "جستجو مقاله های مختلف ..."
  );
  ...
  const getHomePageCourses = async () => {
    let result = await Axios.get(
      `${MainUrl}api/term/list?pagenumber=1&pagesize=8`
    );
    setCourseData(result.data.result.terms);
  };
  const getFavCourses = async () => {
    let result = await Axios.get(
      `${MainUrl}api/term/list?pagenumber=1&pagesize=4`
    );
    setFavCourseData(result.data.result.terms);
  };
  const get6BlogData = async () => {
    let result = await Axios.get(
      `${MainUrl}api/news/list?pagenumber=1&pagesize=6`
    );
